...
class HomeViewController: TableViewController {
    ...
    override func makeUI() {
        ...
        tableView.register(cellWithClass: TrendingUserCell.self)
        tableView.register(cellWithClass: TrendingRepositoryCell.self)
    ...
    override func bindViewModel() {
        ...
        let dataSource = RxTableViewSectionedReloadDataSource<SearchSection>(configureCell: { dataSource, tableView, indexPath, item in
            ...
            case .trendingRepositoriesItem(let cellViewModel):
                let cell = tableView.dequeueReusableCell(withClass: TrendingRepositoryCell.self, for: indexPath)
                cell.bind(to: cellViewModel)
                return cell
            case .trendingUsersItem(let cellViewModel):
                let cell = tableView.dequeueReusableCell(withClass: TrendingUserCell.self, for: indexPath)
                cell.bind(to: cellViewModel)
                return cell
            case .repositoriesItem(let cellViewModel):
                let cell = tableView.dequeueReusableCell(withClass: RepositoryCell.self, for: indexPath)
                cell.bind(to: cellViewModel)
                return cell
